...
def train_and_evaluate_cnn_with_weights(train_dataset, val_dataset, test_dataset, VOCAB_SIZE, PAD_IDX, class_weights_tensor, best_cnn_params, label_encoder, device, model_save_path='models/cnn_model_with_class_weights.pt'):
    ...
    test_loss_cnn_with_weights, test_acc_cnn_with_weights, test_preds_cnn_with_weights, test_labels_cnn_with_weights = evaluate_model(final_cnn_model_with_weights, final_cnn_test_loader_with_weights, final_criterion_with_weights, device)
    print(f'\nFinal CNN Test Loss: {test_loss_cnn_with_weights:.4f} | Final CNN Test Acc: {test_acc_cnn_with_weights*100:.2f}%')
    print("\nCNN Model Performance on Test Set (WITH Class Weighting):")
    cnn_report = classification_report(test_labels_cnn_with_weights, test_preds_cnn_with_weights, target_names=label_encoder.classes_, output_dict=True)
    print(classification_report(test_labels_cnn_with_weights, test_preds_cnn_with_weights, target_names=label_encoder.classes_))

    # Ensure the directory exists before saving
    os.makedirs(os.path.dirname(model_save_path), exist_ok=True)
    torch.save(final_cnn_model_with_weights.state_dict(), model_save_path)
    print(f"CNN model with class weights saved to {model_save_path}")

    return cnn_report, test_labels_cnn_with_weights, test_preds_cnn_with_weights

def train_and_evaluate_lstm_with_weights(train_dataset, val_dataset, test_dataset, VOCAB_SIZE, PAD_IDX, class_weights_tensor, best_lstm_params, label_encoder, device):
    """
    Trains the final LSTM model with class weights and best hyperparameters,
    and evaluates it.
    """
    print("\nTraining final LSTM model (With Weights)...")
    final_lstm_model_with_weights = LSTMClassifier(VOCAB_SIZE, best_lstm_params['embedding_dim'], best_lstm_params['hidden_dim'],
                                      NUM_SENTIMENT_CLASSES, best_lstm_params['n_layers'],
